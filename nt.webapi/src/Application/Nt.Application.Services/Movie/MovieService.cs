...
public class MovieService : ServiceBase, IMovieService
{
    ...
    public async Task<List<MovieEntity>> SearchMovie(string partialTitle,int maxCount = -1)
    {
        ...
        var currentMovieCollection = maxCount == -1 ? result : result.Take(maxCount);
        var resultCollection = new List<MovieEntity>(currentMovieCollection.Count());

        foreach (var movieItem in currentMovieCollection)
        {
